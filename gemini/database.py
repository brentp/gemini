...
def create_tables(path, effect_fields=None):
    ...
    for table in db:
        ...
        cols = [Column(c[0], lookup[c[1].lower()]) for c in db[table]]
    ...
    e = create_engine(get_path(path), isolation_level=None)
    metadata = MetaData(bind=e)

    session = create_session(bind=e, autocommit=False, autoflush=False)
    mapped = {}
    for tbl in sorted(db):
        t = Table(tbl, metadata, *db[tbl])
        mapped[tbl] = t
...
def create_sample_table(cursor, metadata, args):
    NUM_BUILT_IN = 6
    cols = [Column("sample_id", Integer, primary_key=True)]
...
def insert_variation(session, metadata, buffer):
    ...
    # TODO: use raw connection to get faster insert?
    """
    ...
    print "XXXXX"
    print set(cols) - set(buffer[0].keys())
    print set(buffer[0].keys()) - set(cols)
